...
const ItemList = ({items, onItemSelect}) => { // Accept items as a prop
      ...
      <ul className="space-y-4">
        {sortBy === 'group' ? (
              ...
              <ul className="space-y-2">
                {group.items.map((item, index) => (
                  <Item
                    ...
                    onSelect={() => onItemSelect(item)} // Trigger onItemSelect when item is clicked
          ...
          sortedItems.map((item, index) => (
            <Item
              key={item.id || `${item.name}-${index}`}   // Unique key using item id
              name={item.name}
              quantity={item.quantity}
              category={item.category}
              onSelect={() => onItemSelect(item)}   // Trigger onItemSelect when item is clicked
